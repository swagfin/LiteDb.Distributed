...
using DistributedLiteDb.Core.Collections;
using DistributedLiteDb.Core.Models;
using DistributedLiteDb.Core.SampleEntities;
...
namespace DistributedLiteDb.Tests;
...
public sealed class ClusterReplicationTests
{
    ...
    private sealed class TestCluster : IAsyncDisposable
    {
        ...
        public TestCluster()
        {
            _rootPath = Path.Combine(Path.GetTempPath(), "DistributedLiteDb.Tests", Guid.NewGuid().ToString("N"));
